...
class VirtualMouseSystem:
    def __init__(self):
        try:
            ...
            # Tambahkan variabel untuk tracking pose sebelumnya
    ...
    # [Previous methods remain unchanged: setup_logging, load_model, calculate_finger_distances, predict_pose, smooth_prediction]
    ...
    def execute_mouse_action(self, pose, hand_center):
        """Execute mouse actions with single execution control"""
        try:
            ...
            # Kontrol eksekusi aksi
            if pose != self.previous_pose:
                self.action_executed = False
                if self.drag_active and pose != 'fist':
                    pyautogui.mouseUp()
                    self.drag_active = False

            # Execute based on pose with single execution control
            if pose == 'v_pose':
                pyautogui.moveTo(screen_x, screen_y, duration=0.1)
            elif pose == 'middle_finger' and not self.action_executed:
                pyautogui.click()
                self.action_executed = True
            elif pose == 'index_finger' and not self.action_executed:
                ...
                pyautogui.moveTo(screen_x, screen_y, duration=0.1)
            elif pose == 'palm' and self.drag_active:
                pyautogui.mouseUp()
                self.drag_active = False
                self.action_executed = True
