...
CAMERA_INDEX = 0
POSES = ['fist', 'palm', 'index', 'mid', 'v_pose', 'two_finger_closed', 'random']
HANDS = ['left', 'right']
    ...
    for pose in POSES:
        for hand in HANDS:
            for distance in DISTANCES:
                path = os.path.join(DATASET_PATH, pose, hand, distance)
...
def capture_images():
    ...
    logging.info("Controls: 'p': change pose, 'h': change hand, 'd': change distance, 'c': capture image, 'q': quit")
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            cv2.putText(frame, f"Pose: {pose}", (10, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            if key == ord('p'):
                ...
                img_name = f"{pose}_{hand}_{distance}_{count}{IMAGE_EXTENSION}"
                img_path = os.path.join(DATASET_PATH, pose, hand, distance, img_name)
